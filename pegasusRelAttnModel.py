...
class PegasusRelConfig(PegasusConfig):
    def __init__(
        self,
...
class PegasusRelAttention(PegasusAttention):
...
class PegasusRelDecoderLayer(PegasusDecoderLayer):
    def __init__(self, config: PegasusRelConfig):
        super().__init__(config)
        self.encoder_attn = PegasusRelAttention(
            self.embed_dim,
...
class PegasusRelDecoder(PegasusDecoder):
    """
    ...
    def __init__(
        self, config: PegasusRelConfig, embed_tokens: Optional[nn.Embedding] = None
    ):
        super().__init__(config, embed_tokens)
        self.layers = nn.ModuleList(
            [PegasusRelDecoderLayer(config) for _ in range(config.decoder_layers)]
        )
        self.init_weights()
    def forward(
        self,
...
class PegasusRelModel(PegasusModel):
    def __init__(self, config: PegasusRelConfig):
        super().__init__(config)
        self.decoder = PegasusRelDecoder(config, self.shared)
        self.init_weights()
        ...
        if config.rel_attn_type == "trained":
            ...
            self.rel_k_proj.weight.data.fill_(1)
            self.rel_k_proj.bias.data.fill_(0)
            self.rel_q_proj = nn.Linear(config.d_model, config.d_model)
            self.rel_q_proj.weight.data.fill_(1)
            self.rel_k_proj.bias.data.fill_(0)
        # ## zero-shot
...
class PegasusRelForConditionalGeneration(PegasusForConditionalGeneration):
    ...
    def __init__(self, config: PegasusRelConfig):
        super().__init__(config)
        self.model = PegasusRelModel(config)
        self.init_weights()
    def forward(
        self,
        input_ids=None,
